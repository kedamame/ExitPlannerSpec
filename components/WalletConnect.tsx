...
export function WalletConnect({
    ...
    fetch(`/api/wallet-tokens?address=${address}`)
      ...
      .then((data) => {
        ...
        const allKnown = new Map<string, MergedToken>([
          ...Array.from(KNOWN_ETH_MAP.entries()).map(([addr, t]) => [
            addr,
            { address: addr, symbol: t.symbol, name: t.name, decimals: t.decimals, chain: 'eth' as Chain },
          ] as const),
          ...Array.from(KNOWN_BASE_MAP.entries()).map(([addr, t]) => [
            addr,
            { address: addr, symbol: t.symbol, name: t.name, decimals: t.decimals, chain: 'base' as Chain },
          ] as const),
        ])
      ...
      .catch(() => {
        const allKnown: MergedToken[] = [
          ...Array.from(KNOWN_ETH_MAP.values()).map((t) => ({
          ...
          ...Array.from(KNOWN_BASE_MAP.values()).map((t) => ({
            address: t.address.toLowerCase(),
            symbol: t.symbol,
            name: t.name,
            decimals: t.decimals,
  ...
  useEffect(() => {
    if (fallbackList.length === 0 || !address || fallbackBalancesLoading) {
      return
    }
    if (ethContracts.length > 0 && !ethBalances) {
      return
    }
    if (baseContracts.length > 0 && !baseBalances) {
      return
    }

    setLoading(true)

    function extractNonZero(tokenList: MergedToken[], balances: typeof ethBalances) {
      return tokenList
        .map((t, i) => {
  ...
  const preferredConnector = useMemo(() => {
    ...
    if (isInMiniApp) {
      return available.find((connector) => connector.id === 'injected') ?? available[0]
    }
  ...
  const isLoadingBalances = loading || (fallbackList.length > 0 && fallbackBalancesLoading)
        ...
        {isLoadingBalances ? (
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-600 mb-2">
              Tokens are sampled from supported Ethereum and Base sources.
            </p>
  ...
  return (
    <div className="flex flex-col gap-3 w-full max-w-lg">
      <button
        disabled={!preferredConnector || isDetectingMiniApp || isPending}
        onClick={() => preferredConnector && connect({ connector: preferredConnector })}
        className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-800 disabled:text-gray-500 text-white rounded-xl px-6 py-3 font-medium transition"
      >
      ...
      {!isInMiniApp && connectors.length > 0 && (
        <p className="text-center text-xs text-gray-500">
          {connectors
            .filter((connector) => connector.type !== 'mock')
